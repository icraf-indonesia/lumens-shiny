...
Pre-QuES (Preliminary Quantification of Ecosystem Services), a land use change assessment module, quantifies land cover types and their changes over time, identifies specific land use change trajectories, breaks down changes by administrative or planning units, and visualises land use dynamics through maps and Sankey-charts, providing key information to identify potential drivers of land use change.
...
2. **Planning Unit Map**: A complete shapefile dataset representing administrative or management zones. The dataset includes all necessary file components (.shp, .dbf, .shx, and .prj).
3. **Land Use/Cover Lookup Table**: A CSV file describing raster values with their corresponding land cover classes.
4. **Trajectory Rules**: A CSV file defining rules for classifying land use change trajectories.

## Using the Pre-QuES Module

Follow these steps to use the module:

![](pre_ques_gui_entry.gif){style="max-width:80%;height=auto;"}
