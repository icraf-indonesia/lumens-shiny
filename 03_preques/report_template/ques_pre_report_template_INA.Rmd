...
for (i in names(ques_pre_output[["pu_level"]])) {
  ...
  print(
    render_dt_table(
      ques_pre_output[["pu_level"]][[i]][["luc_top_pu"]]
    )
  )
  cat("\n\n")
...
plot_categorical_rst_mapview(traj_map, lookup_traj, paste0("Land Use-Cover Trajectory in ", time_1, "-", time_2))
```
...
render_child <- function(results_list, i) {
  res = knitr::knit_child(
   text = c(
     ...
     "```{r}",
     "#| echo: false",
     "render_dt_table(results_list[[i]][['traj_tbl_pu']])",
     # "mutate_if(is.numeric, easy_to_read_numbers)",
     # "  kable() %>%",
     # "  kable_styling(bootstrap_options = c('striped', 'hover'))",
     "```",
