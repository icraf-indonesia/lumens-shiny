...
```{r zone_map}
plot_categorical_raster(admin_, "Peta Unit Perencanaan.png")
```
...
Luas lanskap yang dianalisis pada laporan ini adalah `r scales::label_comma()(as.numeric(sum_t1))` hektare.
...
traj_tbl <- traj_analysis_output$landscape_level$table_traj_area %>% 
  mutate(
    !!paste0("%") := round((.[[3]] / sum(.[[3]], na.rm = TRUE)) * 100, 2)
  )
