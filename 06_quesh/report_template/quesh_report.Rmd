...
In ", params$t1, ", the predominant soil erosion class was ",
         ...
         "%). These findings highlight changes in soil erosion patterns and intensities over the study period.")
} else {
  paste0("The QuES-H analysis was conducted over an area of interest spanning ",
         format(round(params$summary_data$total_area, 2), big.mark = ","),
...
```{r Land Cover, echo=FALSE, fig.align='left', fig.keep='all', results='asis', message=FALSE, warning=FALSE}
if (params$multiseries == "two_step"){
  lc <- params$landcover
  ...
  res = knitr::knit_child(
    text = c(
      ...
      "plot_categorical_raster(lc[[i]], type = type)",
      "```"
    ),
  ...
  cat("\n")
  plot_categorical_raster(params$landcover, type = "landcover")
}
```
...
plot_categorical_raster(params$pu, type = "planning unit")
```
...
```{r C Factor, echo=FALSE, fig.width=12, fig.align='left', fig.keep='all', results='asis', message=FALSE, warning=FALSE}
if (params$multiseries == "two_step"){
  c <- params$c
  t <- list(params$t1, params$t2)
...
```{r Soil Erosion Estimation Map, echo=FALSE, fig.align='left', fig.keep='all', results='asis', message=FALSE, warning=FALSE}
type <- "erosion"
...
if (params$multiseries == "two_step"){
  ...
  res = knitr::knit_child(
    text = c(
      "#### Map of Soil Erosion `r t[i]`",
      "Soil Erosion generated from RUSLE calculation",
      "```{r, echo=FALSE, message=FALSE, warning=FALSE}",
      "plot_categorical_raster(a[[i]], type = type)",
      "```"
    ),
  envir = rlang::env(results_list = results_list, i = i),
  quiet = TRUE
  ...
  plot_categorical_raster(params$a, type = type)
}
```

#### Table of Soil Erosion Rates
...
### Soil Erosion at Planning Unit Level {.tabset}

```{r soil_erosion_pu_level, echo=FALSE, results='asis'}
render_child <- function(e_pu_df, time_point) {
  res = knitr::knit_child(
    text = c(
      "#### Table of Soil Erosion in `r time_point`",
      "```{r}",
      "#| echo: false",
      "datatable(e_pu_df %>% mutate(across(where(is.numeric), scales::comma)),",
      "  caption = htmltools::tags$caption(",
      "    style = 'caption-side: top; text-align: left; color:black; font-size:150%;',",
      "    paste('Soil Erosion (Ha) in', time_point)))",
      "```"
    ),
    envir = rlang::env(e_pu_df = e_pu_df, time_point = time_point),
    quiet = TRUE
  )
  cat(res, sep = '\n')
  cat("\n")
...
if (params$multiseries == "two_step") {
  render_child(params$e_pu_df[[1]], params$t1)
  render_child(params$e_pu_df[[2]], params$t2)
} else {
  render_child(params$e_pu_df, "Analysis Period")
}
```

```{r soil_erosion_changes, echo=FALSE, results='asis', fig.width=12, fig.align='left', warning=FALSE, message=FALSE}
render_child <- function(content_type) {
  res = knitr::knit_child(
    text = c(
      paste("####", content_type),
      "```{r}",
      ...
      "#| warning: false",
      "#| message: false",
      if (content_type == "Map of Soil Erosion Changes") {
        "plot_categorical_raster(params$e_diff, type = 'erosion difference')"
      } else if (content_type == "Table of Soil Erosion Changes at Landscape Level") {
        c(
          ...
          "    style = 'caption-side: top; text-align: left; color:black; font-size:150%;',",
          "    paste('Soil Erosion Changes (Ha) at Landscape Level in', params$t1, '-', params$t2)",
          "  )",
          ") %>%",
          "  formatRound(",
          "    columns = 'Percentage (%)',",
      ...
      } else if (content_type == "Table of Soil Erosion Area Changes at Planning Unit Level") {
        c(
          ...
          "  caption = tags$caption(",
          "    style = 'caption-side: top; text-align: left; color:black; font-size:150%;',",
          "    paste('Soil Erosion Area Changes (Ha) at Planning Unit Level in', params$t1, '-', params$t2)",
          "  )",
          ")"
        )
      } else if (content_type == "Table of Soil Erosion Changes at Planning Unit Level") {
        c(
          "e_stats <- params$e_stat %>% ",
          "mutate(across(where(is.numeric), scales::comma))",
          "datatable(",
          "  e_stats,",
          "  colnames = c('Planning Unit', 'Total Increased Erosion', 'Total Decreased Erosion', 'Total Net Erosion Changes', 'Mean Erosion Increased', 'Mean Erosion Decreased', 'Mean Net Erosion Changes'),",
          "  caption = tags$caption(",
          "    style = 'caption-side: top; text-align: left; color:black; font-size:150%;',",
          "    paste('Soil Erosion Changes (Ton/Ha/Year) Statistic at Planning Unit Level in', params$t1, '-', params$t2)",
          "  )",
          ")"
        )
      },
...
if (params$multiseries == "two_step") {
  cat("### Soil Erosion Changes {.tabset}\n\n")
  render_child("Map of Soil Erosion Changes")
  render_child("Table of Soil Erosion Changes at Landscape Level")
  render_child("Table of Soil Erosion Area Changes at Planning Unit Level")
  render_child("Table of Soil Erosion Changes at Planning Unit Level")
}
```
