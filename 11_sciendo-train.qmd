...
```{r}
woe$pu001$unique_transition <- woe$pu001$report %>% 
  distinct(Transition) %>% 
  unlist() %>%
  as.vector()
report_per_transisition <- woe$pu001$report %>% filter(Transition == woe$pu001$unique_transition[10]) %>% filter(Significant == "yes") 
...
```{r}
# separate list from generating text task (1)
woe <- list()
len <- nrow(df_z)
for(i in 1:len){
...
```{r}
# separate list from generating text task (2)
for(i in 1:len){
  cat(
    paste("Planning Unit:", df_z[i, 2])
  )
  cat('\n')
  cat('\n')
  for(j in woe[[paste0("pu", sprintf("%03d", i))]][['unique_transition']]) {
