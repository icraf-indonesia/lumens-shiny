server <- function(input, output, session) {
  ...
  # wd <- reactiveValues(NULL)
  int_con_data <- reactive({ read.csv(input$int_con_file$datapath, header = FALSE) })
  add_val_data <- reactive({ read.csv(input$add_val_file$datapath, header = FALSE) })
  fin_dem_data <- reactive({ read.csv(input$fin_dem_file$datapath, header = FALSE) })
  fin_dem_struc_data <- reactive({ read.csv(input$fin_dem_struc_file$datapath) })
  add_val_struc_data <- reactive({ read.csv(input$add_val_struc_file$datapath) })
  sector_data <- reactive({ read.csv(input$sector_file$datapath, header = FALSE) })
  labour_data <- reactive({ read.csv(input$labour_file$datapath, header = FALSE) })
  land_distribution_data <- reactive({ read.csv(input$land_distribution_file$datapath, header = FALSE) })
  land_use_data <- reactive({ raster(input$land_use_file$datapath) })
  # report_file <- reactiveValues(NULL)
  ...
  output$selected_directory <- renderText({
    wd <- parseDirPath(volumes, input$wd)
    if(length(wd) == 0) {
      return()
    } else {
      paste0("Selected output directory: ", wd)
    }
  })
  
  results <-  eventReactive(input$processTAReg1, {
    int_con.m <- as.matrix(int_con_data())
    add_val.m <- as.matrix(add_val_data())
    sector <- sector_data()
    labour <- labour_data()
    land.distribution <- land_distribution_data()
    land_use <- land_use_data()
    fin_dem <- fin_dem_data()
    int_con <- int_con_data()
    dim<-ncol(int_con.m)
    ...
    # Return a list of the results you want to display
    list(
  ...
  output$PRS_graph <- renderPlot({
    results()$BPD_graph
  })
  
  output$OMPL_graph <- renderPlot({
    results()$BPD_graph
  })
  
  output$IMPL_graph <- renderPlot({
    results()$BPD_graph
  })
  
  output$LMPL_graph <- renderPlot({
    results()$BPD_graph
  })
  ...
  report_content <- reactive({
    ...
    output_file <- paste0("ta_report_", Sys.Date(), ".html")
    output_dir <- "C:/Downloads/Result"
    report_file <- paste(output_dir, output_file, sep = "/")
  ...
  report_content <- reactive({
    params <- list(
      Linkages_table = results()$Linkages_table,
      BPD_graph = results()$BPD_graph 
    )
    output_file <- paste0("ta_report_", Sys.Date(), ".html")
    output_dir <- "C:/Downloads/Result"
    report_file <- paste(output_dir, output_file, sep = "/")
  ...
  observeEvent(input$viewReport, {
    file.show(report_file) # Make sure 'report_file' is defined correctly
  })
}