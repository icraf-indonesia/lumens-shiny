...
```{r input-data, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
reclassify_matrix <- as.matrix(z$data_zone_df[, 1]) %>% 
    cbind(., as.matrix(z$data_zone_df[, 4])) %>%
    rbind(., c(0, NA))
...
reclassify_matrix <- as.matrix(z$data_zone_df[, 1]) %>% 
    cbind(., as.matrix(z$data_zone_df[, 5])) %>%
    rbind(., c(0, NA))
...
reclassify_matrix <- as.matrix(z$data_zone_df[, 1]) %>% 
    cbind(., as.matrix(z$data_zone_df[, 6])) %>%
    rbind(., c(0, NA))
...
reclassify_matrix <- as.matrix(z$data_zone_df[, 1]) %>% 
    cbind(., as.matrix(z$data_zone_df[, 7])) %>%
    rbind(., c(0, NA))
...
z$tb_em_total_10 %>% kbl(caption = "The Largest Source of Emission", escape = F) %>%
  kable_paper("hover", full_width = F)
```
...
z$tb_sq_total_10 %>% kbl(caption = "The Largest Source of Sequestration", escape = F) %>%
  kable_paper("hover", full_width = F)
```
