server <- function(input, output, session) {
  ...
  observeEvent(input$process, {
    ...
    ref <- rasterise_multipolygon(sf_object = ref_data,
    ...
    for (i in 1:n_pu_list) {
      ...
      getwd()
      pu_raster <- rasterise_multipolygon(sf_object = pu_vector, raster_res = c(rv$map_resolution,rv$map_resolution), field = "ID")
      print(pu_raster)
    ...
    writeRaster(PUR, filename=paste0(rv$wd, "PUR_first_phase_result"), format="GTiff", overwrite=TRUE)
    ...
    writeVector(rv$pur_unresolved_vector, filename = file.path(paste0(rv$wd, "/PUR_first_phase_result.shp")), overwrite = TRUE)

    write.table(data_attribute, paste0(rv$wd, "/PUR_attribute.csv"), quote=FALSE, row.names=FALSE, sep=",")
    #PUR_dbfinal <- PUR_dbfinal |> select(-ID_rec)
    write.table(PUR_dbfinal, paste0(rv$wd, "/PUR_dbfinal.csv"), quote=FALSE, row.names=FALSE, sep=",")
    ...
    rmarkdown::render(
      input = "../report_template/PUR1_report.Rmd",
