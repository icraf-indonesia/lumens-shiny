...
analysis_log <- tribble(
  ...
  "Transition Matrix Folder", scen_list,
  "Session Summary", ""
...
This report presents the results of an analysis based on the QUES-C database and scenario builder modified by SCIENDO.
  ...
  cat(
      knitr::knit_child(
        text = c(
          "```{r}",
          "#| echo: false",
          "scenario_modif %>% select(period, zone, lc1, lc2, area) %>% datatable()",
          "```"
        ), quiet = TRUE
      ), sep = "\n"
    )
