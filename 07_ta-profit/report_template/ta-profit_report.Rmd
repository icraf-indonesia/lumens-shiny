...
dt_opcost <- params$opcost_table %>%
  mutate(
    # Convert factors to numeric
    opcost = as.numeric(as.character(params$opcost_table$opcost)),
    opcost_log = as.numeric(as.character(params$opcost_table$opcost_log)),
    emrate = as.numeric(as.character(params$opcost_table$emrate)),
    cum_emrate = as.numeric(as.character(params$opcost_table$cum_emrate)),
    cum_emrate2 = as.numeric(as.character(params$opcost_table$cum_emrate2))
  ) %>%
  mutate(
    # Round to two decimal places for columns like 'opcost' and 'opcost_log'
    opcost = round(opcost, 3),
    opcost_log = round(opcost_log, 3),
    
    # Format large numbers with commas for readability
    emrate = scales::comma(emrate, accuracy = 0.000001),
    cum_emrate = scales::comma(cum_emrate, accuracy = 0.000001),
    cum_emrate2 = scales::comma(cum_emrate2, accuracy = 0.000001)
  ) %>%
  mutate(across(where(is.numeric), ~ scales::number(.x, big.mark = ","))) 

DT::datatable(dt_opcost, rownames = FALSE) 
```

## Opportunity Cost Curve {.tabset}
...
### Table
```{r, echo=FALSE}
opcost_curve_table <- params$opcost_curve_table %>%
  mutate(
    opportunity_cost = round(params$opcost_curve_table$opportunity_cost, 3),
    emission_rate = scales::comma(params$opcost_curve_table$emission_rate, accuracy = 0.000001),
  ) %>%
  mutate(across(where(is.numeric), ~ scales::number(.x, big.mark = ","))) 

opcost_curve_table$order <- NULL

DT::datatable(opcost_curve_table, rownames = FALSE) 
```
