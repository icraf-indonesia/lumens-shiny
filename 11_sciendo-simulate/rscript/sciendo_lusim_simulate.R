...
###############################
proj.file="D:/OneDrive - CIFOR-ICRAF/ICRAF/LUMENS/LUMENS for Brazil ToT/lumens_sciendo_update/lumens_sciendo_update.lpj"
SCIENDO_LUCM_index="1_SCIENDO_lucm_2015-2020_PU"
n_rep = 2
tpm_file = "scenario_tpm_1.csv"
###############################
...
# divide the tpm file into single steps
tpm <- read.csv(file.path(rate_dir, tpm_file))
zone_ids <- unique(tpm$zone_id)

for (zone in zone_ids) {
  subset_tpm <- tpm[tpm$zone_id == zone, ]
  subset_tpm <- subset_tpm[, -which(names(subset_tpm) == "zone_id")]
  subset_tpm <- subset_tpm[order(subset_tpm[[1]]), ]
  colnames(subset_tpm) <- c('From*', 'To*', 'Rate')
  subset_tpm <- subset_tpm[subset_tpm$`From*` != subset_tpm$`To*`, ]
  subset_tpm$Rate <- format(as.numeric(subset_tpm$Rate), scientific = FALSE)
  
  write.csv(subset_tpm, paste0(rate_dir, '/',"Single_step00000", zone, ".csv"), row.names = FALSE, quote = FALSE)
}
...
file_init_lc <- "/landuse_1.tif"
zone <- "/zone.tif"
DINAMICA_exe<-paste0(Sys.getenv("ProgramFiles"), "\\Dinamica EGO 6\\DinamicaConsole6.exe")
if (file.exists(DINAMICA_exe)){
  urlDINAMICAConsole = DINAMICA_exe
} else{
  DINAMICA_exe<-paste0(Sys.getenv("ProgramFiles(x86)"), "\\Dinamica EGO 6\\DinamicaConsole6.exe")
  urlDINAMICAConsole = DINAMICA_exe
}
...
# outputfolder <-"D:/MW/KubuRaya_1/SCIENDO/2_SCIENDO_lucm_2009_2020_Administrative.maps/result/projectedlandscape"
# setwd(outputfolder)
# listprojlc <- list.files(outputfolder, 
#                          full.names=TRUE, 
#                          pattern=".tif$")
# editorprojlc<-data.frame(file=listprojlc, select=1)
# 
# selectedprojlc<-subset(editorprojlc, select==1)
# projlc<-as.character(selectedprojlc$file)
# nprojlc <- length(projlc)
# 
# for (i in 1:nprojlc) {
#   assign(paste("landuse",i, sep=""),raster(paste("Landscape000",i,".tif",sep="")))        
# }  
# 
# 
# # lookup_lc <- read.table(Look_up_table_land_use, header=TRUE, sep=",",)
# lookup_lc <- read.table("D:/MW/DATA/Kalbar/Tabular/Tutupan lahan.csv", header=TRUE, sep=",")
...
# }
# landuse_t1<-raster("D:/MW/KubuRaya_1/SCIENDO/2_SCIENDO_lucm_2009_2020_Administrative.maps/landuse_1.tif")
# landuse_t2<-raster("D:/MW/KubuRaya_1/SCIENDO/2_SCIENDO_lucm_2009_2020_Administrative.maps/landuse_2.tif")
# 
# #Landuse 1 map
...
# 
# #original landuse map
# plot.LUt1<-gplot(landuse_t1, maxpixels=100000) + geom_raster(aes(fill=as.factor(value))) +
...
# plot.LUt2<-gplot(landuse_t2, maxpixels=100000) + geom_raster(aes(fill=as.factor(value))) +
#   coord_equal() + ColScale.lu +
#   theme(plot.title = element_text(lineheight= 5, face="bold")) +
#   theme( axis.title.x=element_blank(),axis.title.y=element_blank(),
#          panel.grid.major=element_blank(), panel.grid.minor=element_blank(),
