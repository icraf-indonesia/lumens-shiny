...
lc_t1 <- params$inputs$lc_t1_path %>% terra::rast() %>%
...
for(i in list_luc){
  ...
  cat("### Projected land cover at T", counter, "\n\n")
  cat(
    knitr::knit_child(
      text = c(
        "```{r}",
...
## Land Use/Change Change Lookup Table
...
colnames(all_freq) <- c("ID", paste0("T", 1:(ncol(all_freq)-1)))
df_merge <- df_lc[, 1:2]
all_freq <- all_freq %>% 
  mutate_if(is.character, as.numeric) %>% 
  left_join(df_merge, by = "ID") %>%
  dplyr::arrange(ID) %>%
  dplyr::relocate(LC, .before = T1)
all_freq %>% kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
