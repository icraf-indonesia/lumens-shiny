...
lc_freq_table <- multiple_lc_freq_combined(lc_dir, PU = "NO")
...
all_freq_pu_list <- multiple_lc_freq_combined(lc_dir, PU = "YES", zone = zone, split = "YES")

for (i in names(all_freq_pu_list)) {
  cat("### ", i, "\n\n")
  
  tbl <- DT::datatable(
    all_freq_pu_list[[i]],
    caption = paste("Projected Land Use/Cover in", i),
    options = list(
      dom = 't',
      pageLength = nrow(all_freq_pu_list[[i]])
    )
  )
