...
suit_by_factors <- rast(file_name_suit_factors_tif)
```
...
analysis_log <- tribble(
  ...
  "3. Land suitability map (polygon)", params$file_name_land_suit_shp,
...
```{r climate-soil-data}
climate_soil_data %>% 
  select(-c("raster_path", "name_parameter")) %>%
  datatable()
```

## Maps of agroclimatic, soil, and terrain conditions

```{r agroclimatic-maps, fig.height=8, fig.width=16}
...
suitability_map <- leaflet(suitability_polygon) %>% 
  ...
  addPolygons(
   ...
   addPolygons(
      ...
      # Set the fill color based on 'suitability'
      fillOpacity = 1,
      color = "grey",
      weight = 0.1,
      smoothFactor = 1,
...
```{r suitability-factors, fig.height=8, fig.width=16}
plot_suitability_factors(suit_by_factors)
```
