...
render_child <- function(results_list, i) {
  res = knitr::knit_child(
   text = c(
     "### `r i` {.tabset}",
     "",
     "#### Table of Weight of Evidence Coefficients in `r i`",
     "```{r}",
     "#| echo: false",
     "weight_report %>%",
     "  select(-X) %>%",
     ...
     "    Range = paste(Range_Lower_Limit., '->', Range_Upper_Limit.),",
...
## Table of Weight of Evidence

```{r child_woe_report, echo=FALSE, fig.keep='all', results='asis'}
listWoeReport <- params$inputs$output_dir %>% list.files(full.names=TRUE, pattern="weight_report*")
csv_reports <- as.character(listWoeReport)
for(r in csv_reports){
  weight_report <- read.csv(csv_reports[r])
  render_child(results_list = weight_report, i = r)
}
```
