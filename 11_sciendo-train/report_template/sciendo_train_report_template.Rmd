...
collinearity <- tryCatch({
  ...
  warning("Multicollinearity analysis failed: ", e$message)
  NULL
})

if (!is.null(collinearity$zero_variance_vars) && length(collinearity$zero_variance_vars) > 0) {
  cat(paste0("\n> **Note:** The following variables were excluded from multicollinearity analysis due to zero variance (constant values): **", 
...
if (!is.null(collinearity$vif_table) && nrow(collinearity$vif_table) > 0) {
  ...
  cat("_Insufficient data to compute VIF table._\n")
}
```
...
if (!is.null(collinearity$correlation_plot)) {
  ...
  cat("_Collinearity matrix cannot be displayed._\n")
}
```
