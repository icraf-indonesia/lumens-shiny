...
## Land-Use/Cover Map and Planning Unit {.tabset}
...
listWoeReport <- params$woe_report_path %>% list.files(full.names=TRUE, pattern="weight_report*")
woe <- params$woe_list
df_zone <- df_pu
report_per_transition_per_variable <- data.frame(information = "No significant weight coeffients above five were detected")
vec <- c()
for(l in listWoeReport) {
  number <- gsub(paste0(params$woe_report_path, "/weight_report"), "", l) %>% 
    substr(1, 2) %>% 
    as.numeric() # error if PU > 99
  vec <- c(vec, number)
}
```
...
for(counter in 1:length(vec)){
  i <- vec[counter]
  cat("### ", df_zone[i, 2], "\n\n")
  cat(paste("File location:", listWoeReport[counter]))
  cat('\n\n')
  
  unique_trans <- woe[[paste0("pu", sprintf("%03d", i))]][['unique_transition']]
  for(j in 1:length(unique_trans)) {
    report_per_transition <- woe[[paste0("pu", sprintf("%03d", i))]][['report']] %>% 
      dplyr::filter(Transition == unique_trans[j] & Significant == "yes" & Weight_Coefficient > 5)
    ...
    if(length(unique_variable) == 0) {
      cat(
          paste(
            "Transition:", unique_trans[j],
            sep = " ")
        )
      cat('\n')
      cat('\n')
      ...
      for(k in 1:length(unique_variable)) {
        report_per_transition_per_variable <- report_per_transition %>% filter(Variable. == unique_variable[k]) %>% 
          dplyr::select(
