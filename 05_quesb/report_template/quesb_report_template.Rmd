...
source("../rscript/ques_biodiv_functions.r")
```
...
analysis_log <- tribble(
  ...
  if (is_two_periods) c("3. Second Landcover map", params$inputs$lc_t2_path, "4. Year", as.character(params$inputs$t2)) else NULL,
  "5. Landcover lookup",   params$inputs$lulc_lut_path,
  "6. Contrast table",     params$inputs$contab_path,
  "7. NoData land-cover class", as.character(params$inputs$raster.nodata),
  "8. Number of sampling points (n)", as.character(params$inputs$sampling_points),
  "9. Moving window radius (metres)", as.character(params$inputs$window_size),
  "10. Moving window shape", as.character(params$inputs$window.shape),
  "11. Fragstats config file", params$inputs$fca_path,
  "12. Fragstats directory", params$inputs$fragstats_path,
  "Output Files", "",
  "Output Workspace",      paste0(getwd(), params$output_dir),
  if (is_two_periods) {
    c("1. Focal Area Map (t1)", file.path(params$output_dir, params$dir_focal_area_t1),
      "2. Focal Area Map (t2)", file.path(params$output_dir, params$dir_focal_area_t2),
      "3. TECI Map (t1)", file.path(params$output_dir, params$dir_teci_map_t1),
      "4. TECI Map (t2)", file.path(params$output_dir, params$dir_teci_map_t2),
      "5. TECI Difference Map", file.path(params$output_dir, params$dir_teci_difference),
      "6. DIFA Table (t1)", file.path(params$output_dir, params$dir_difa_table_t1),
      "7. DIFA Table (t2)", file.path(params$output_dir, params$dir_difa_table_t2),
      "8. Sampling Grid", file.path(params$output_dir, params$dir_sampling_grid)
    )
  } else {
    c("1. Focal Area Map", file.path(params$output_dir, params$dir_focal_area_),
...
teci_diff_map <- file.path(params$output_dir, params$dir_teci_diff_category) %>% rast()
...
ggplot(difa_table_t1, aes(x = TECI, y = FocalArea)) +
  geom_area(alpha = 0.8) +
...
ggplot(difa_table_t2, aes(x = TECI, y = FocalArea)) +
  geom_area(alpha = 0.8) +
...
difa_combined <- bind_rows(difa_table_t1, difa_table_t2)

ggplot(difa_combined, aes(x = TECI, y = FocalArea, color = as.factor(Time))) +
  geom_line(size = 1) +
