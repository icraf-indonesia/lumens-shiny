...
file.copy("05_quesb/report_template/quesb_report_template.Rmd",
          to = file.path(output_dir, "QuES_B_report.Rmd"), overwrite = TRUE)

session_log <- format_session_info_table()

# Prepare parameters for report rendering
fca_path <- if (is.null(fca_path)) "05_quesb/rscript/teciuf.fca" else fca_path

if (is.null(fragstats_path)) {
...
params <- list(
  ...
  dir_focal_area = basename(quesb_result$path_focal_area),
...
rmarkdown::render(
  input = file.path(output_dir, "QuES_B_report.Rmd"),
