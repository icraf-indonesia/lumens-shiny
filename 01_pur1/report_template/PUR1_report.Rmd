...
```{r Unresolved-Cases-Zone-Map, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10, fig.height=8}
n_colors <- length(unique(params$pur_unresolved_vector$Rec_phase2))
colors <- brewer.pal(n = min(n_colors, 8), name = "Set2")
if (n_colors > 8) {
  colors <- colorRampPalette(colors)(n_colors)
}

ggplot() +
  geom_sf(data = params$pur_unresolved_vector, aes(fill = Rec_phase2), color = "white", size = 0.1) +
  scale_fill_manual(values = colors, na.value = "grey50", name = NULL) +
  theme_minimal() +
  theme(
...
summary_unresolved_display1 <- params$database_unresolved_out 
