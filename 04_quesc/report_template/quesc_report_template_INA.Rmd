...
render_dt_table(
  s$summary_df,
  "Tabel Ringkasan Perhitungan Emisi & Sekuestrasi GRK"
  )
```

## Ringkasan Perhitungan Emisi GRK Berdasarkan Unit Perencanaan {.tabset}
...
render_dt_table(
  s$zone_carbon,
  caption = "Tabel Ringkasan Perhitungan Emisi & Sekuestrasi GRK di Tingkat Unit Perencanaaan"
  )
...
render_dt_table(
  z$data_zone_df,
  "Tabel Ringkasan Perhitungan Emisi & Sekuestrasi GRK Antar Periode di Tingkat Unit Perencanaaan"
  )
```
...
plot_categorical_raster(lc_t1, filename = paste0("Land Cover-Use ", params$p1, ".png"))
```

## Peta Tutupan/Penggunaan Lahan pada `r params$p2`

```{r Land-Use/Cover Map T2, echo=FALSE, message=FALSE, warning=FALSE}
plot_categorical_raster(lc_t2, filename = paste0("Land Cover-Use ", params$p2, ".png"))
```

## Peta Unit Perencanaan

```{r Planning Unit, echo=FALSE, message=FALSE, warning=FALSE}
plot_categorical_raster(zone_rst, "Peta Unit Perencanaan.png")
```
...
## Estimasi Emisi dan Sekuestrasi {.tabset}
...
```{r emission_map, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%', out.height='500px'}
# plot_quesc_results(
#   map = params$map_em,
...
terra::time(map_em) <- NULL
html_caption_em <- "Sekuestrasi (tonne CO<sub>2</sub>-eq ha<sup>-1</sup>)"

plot_continuous_raster(
  raster_data = map_em,
  low_color = "#FFCC66",
  high_color = "#FF0000",
  caption = html_caption_em,
  raster_layer = "Sebaran Emisi GRK"
)
```
...
```{r sequestration_map, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%', out.height='500px'}
# plot_quesc_results(
#   map = params$map_sq,
...
terra::time(map_sq) <- NULL
html_caption_sq <- "Sekuestrasi (tonne CO<sub>2</sub>-eq ha<sup>-1</sup>)"

plot_continuous_raster(
  raster_data = map_sq,
  low_color = "#FFCC66",
  high_color = "#000033",
  caption = html_caption_sq,
  raster_layer = "Sebaran Sekuestrasi GRK"
)
```

### Laju Emisi dan Sekuestrasi {.tabset}
...
## Sumber Emisi & Sekuestrasi {.tabset}
...
render_dt_table(tbl_largest_em)
