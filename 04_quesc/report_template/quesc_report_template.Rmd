...
zone_sf1 <- params$inputs$admin_z_path %>% st_read(quiet = TRUE)
zone_sf <- st_cast(zone_sf1, "MULTIPOLYGON")

map_res <- res(params$map_c1 %>% raster())
zone <- zone_sf %>% rasterise_multipolygon_quesc(raster_res = map_res, field = paste0(colnames(st_drop_geometry(zone_sf[1]))))
...
datatable(z$tb_em_total_10_summary)
...
datatable(z$tb_sq_total_10_summary)
