...
z$data_zone_df %>% kbl(caption = "Carbon Stock and Net Emission Summary Across Time Periods by Planning Unit", escape = F) %>%
  kable_paper("hover", full_width = F)
```
...
Emission map illustrating a decrease in carbon stock amount (in tonne-C/ha).

```{r emission_map, echo=FALSE}
plot_quesc_results(
  map = params$map_em, 
  legend = "Emission (tonne-C/ha)", 
  low = "#FFCC66", 
  high = "#FF0000"
)
```
...
Sequestration map illustrating an increase in carbon stock amount (in tonne-C/ha).

```{r sequestration_map, echo=FALSE}
plot_quesc_results(
  map = params$map_sq, 
  legend = "Sequestration (tonne-C/ha)", 
  low = "#FFCC66", 
  high = "#000033"
)
```

## Average of Nett Emission Rate
